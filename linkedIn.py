...
def search_and_send_request(keywords, till_page, writer, ignore_list=[]):
    for page in range(1, till_page + 1):
        ...
        time.sleep(5)
        linkedin_urls = driver.find_elements(
        ...
        for index, result in enumerate(linkedin_urls, start=1):
            ...
            if text in ignore_list or text.strip() in ignore_list:
                ...
                continue
            connection_action = result.find_elements(
            ...
            if connection.text == "Connect":
                try:
                    coordinates = (
                        connection.location_once_scrolled_into_view
                    ...
                    connection.click()
                    time.sleep(5)
                    if driver.find_elements(By.CLASS_NAME, "artdeco-button--primary")[
                        0
                    ].is_enabled():
                        driver.find_elements(By.CLASS_NAME, "artdeco-button--primary")[
                            0
                        ].click()
                        writer.writerow([text])
                        print("%s ) SENT: %s" % (index, text))
                    else:
                        driver.find_elements(By.CLASS_NAME, "artdeco-modal__dismiss")[
                            0
                        ].click()
                        print("%s ) CANT: %s" % (index, text))
...
try:
    ...
    if not file_exists:
        writer.writerow(["Connection Summary"])
